...
function generateBranchConfig(branchUpgrades) {
  ...
  branchUpgrades.forEach(upg => {
    ...
    const thisVersion =
      Array.isArray(upg.newVersion) && upg.newVersion.length === 1
        ? upg.newVersion[0]
        : upg.newVersion;
    if (!newVersion.includes(thisVersion)) {
      newVersion.push(thisVersion);
    }
  });
  const groupEligible =
  ...
  for (const branchUpgrade of branchUpgrades) {
    ...
    if (newVersion.length > 1) {
      delete upgrade.commitMessageVersion;
    ...
    upgrade.branchName = handlebars.compile(upgrade.branchName)(upgrade);
    upgrade.branchName = handlebars.compile(upgrade.branchName)(upgrade);
    upgrade.prTitle = upgrade.prTitle || upgrade.commitMessage;
    upgrade.prTitle +=
